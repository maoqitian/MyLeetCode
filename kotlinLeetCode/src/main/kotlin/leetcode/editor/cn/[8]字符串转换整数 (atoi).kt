...
class Solution {
    fun myAtoi(s: String): Int {
        ...
        if (!Character.isDigit(str[0]) && str[0] != '-' && str[0] != '+') return 0
        ...
        //确定遍历 index
        var index = if (!Character.isDigit(str[0])) 1 else 0
        while (index < str.length && Character.isDigit(str[index])) {
            //如果为数字 并且没有超过最大长度
            //计算结果
            res = res * 10 + (str[index++] - '0')
            if (!neg && res > Int.MAX_VALUE) {
                //如果为正数并且大于 int 最大值
                res = Int.MAX_VALUE.toLong()
                break
            }
            if (neg && res > 1L + Int.MAX_VALUE) {
                //如果为负数并且大于 int 最大值
                res = 1L + Int.MAX_VALUE
                break
            }
        }
        return if (neg) (-res).toInt() else res.toInt()
    }
}
